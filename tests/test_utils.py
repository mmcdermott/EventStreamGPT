...
class TestUtilFunctions(MLTypeEqualityCheckableMixin, unittest.TestCase):
    ...
    def test_count_or_proportion(self):
        self.assertEqual(4, count_or_proportion(10, 4), msg="When passed an integer, should return same.")
        self.assertEqual(
            3, count_or_proportion(10, 1/3), msg="When passed a float, should return the proportion."
        )
    ...
    def test_flatten_dict(self):
        ...
        self.assertEqual({'foo': 3, 'bar': 3, 'biz': 4}, flatten_dict({('foo', 'bar'): 3, ('biz',): 4}))

    def test_to_sklearn_np(self):
        self.assertEqual(
            np.array([], dtype=float).reshape(-1, 1),
            to_sklearn_np(pd.Series([], dtype=float))
        )
        self.assertEqual(np.array([[1], [-2], [3], [-4]]), to_sklearn_np(pd.Series([1, -2, 3, -4])))
        self.assertEqual(
            np.array([[1], [2], [3], [4]]),
            to_sklearn_np(pd.Series([1, np.NaN, 'foo', 2, 3, 4]))
        )
...
class TestJSONableMixin(unittest.TestCase):
    def test_save_load(self):
        class Derived(JSONableMixin):
            ...
            def __eq__(self, other): return type(other) is type(self) and vars(other) == vars(self)
        ...
        with TemporaryDirectory() as d:
            ...
            with self.assertRaises(FileExistsError): obj.to_json_file(save_path)
