...
from EventStream.data.config import (
    DatasetConfig, MeasurementConfig, PytorchDatasetConfig
)
        ...
        for C in cases:
            with self.subTest(C['msg']):
                if C.get('should_raise', None) is not None:
                    with self.assertRaises(C['should_raise']): PytorchDatasetConfig(**C['kwargs'])
                else: PytorchDatasetConfig(**C['kwargs'])
...
class TestDatasetConfig(ConfigComparisonsMixin, unittest.TestCase):
    def test_validates_params(self):
        ...
        for kwargs in valid_kwargs: DatasetConfig(**kwargs)
        ...
        for kwargs in invalid_kwargs:
            with self.assertRaises(AssertionError): DatasetConfig(**kwargs)
