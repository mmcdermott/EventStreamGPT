...
class TestVocabulary(MLTypeEqualityCheckableMixin, unittest.TestCase):
    ...
    def test_vocabulary(self):
        ...
        self.assertEqual(vocab.element_types, set([str]))
        ...
        with self.assertRaises(AssertionError, msg="Should error on type mismatches."): vocab[3.2]
        ...
        self.assertEqual(vocab['bar'], 1)
        with self.assertRaises(IndexError, msg="Can't retrieve elements out of range."): vocab[3]
        self.assertEqual(vocab['foo'], 0)

        vocab = Vocabulary(vocabulary=[3.5, 'UNK', 100.0], obs_frequencies=[0.2, 0.3, 0.5])

        self.assertEqual(vocab.element_types, set([float]))
        ...
        with self.assertRaises(AssertionError, msg="Should error on type mismatches."): vocab['fizz_bang']
        ...
        with self.assertRaises(AssertionError, msg="Vocab can't be empty"):
            vocab = Vocabulary([], [])
        with self.assertRaises(AssertionError, msg="Vocab and frequencies must be the same length."):
