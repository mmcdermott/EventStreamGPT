...
class TestEventStreamPytorchDatasetConfig(unittest.TestCase):
    def test_validation(self):
        cases = [
            ...
            {'msg': "Should construct with max_seq_len = 1", 'kwargs': {'max_seq_len': 1, 'min_seq_len': 0}},
            {
                'msg': "Should construct with min_seq_len = max_seq_len",
                'kwargs': {'max_seq_len': 10, 'min_seq_len': 10}
            },
            {'msg': "Should construct with seq_padding_side='left'", 'kwargs': {'seq_padding_side': 'left'}},
            {
...
class TestMeasurementConfig(ConfigComparisonsMixin, unittest.TestCase):
    ...
    def test_add_missing_mandatory_metadata_cols(self):
        ...
        with self.assertRaises(AssertionError): config.add_missing_mandatory_metadata_cols()
    ...
    def test_add_empty_metadata(self):
        ...
        with self.assertRaises(AssertionError): config.add_empty_metadata()
    ...
    def test_to_and_from_dict(self):
        ...
        nontrivial_measurement_metadata_df = pd.DataFrame(
            {'A': [1, 2, 3], 'B': ['a', 'b', 'c']}, index=pd.Index([2, 4, 6], name='col'),
        )
            ...
            'want_dict': {
                **default_dict,
                ...
                'measurement_metadata': {
                    'index': [2, 4, 6], 'columns': ['A', 'B'], 'data': [[1, 'a'], [2, 'b'], [3, 'c']],
                    'index_names': ['col'], 'column_names': [None],
                }
            },
        }, {
...
class TestEventStreamDatasetConfig(ConfigComparisonsMixin, unittest.TestCase):
    def test_validates_params(self):
        valid_kwargs = [
            dict(),
            dict(
        ...
        for kwargs in valid_kwargs: EventStreamDatasetConfig(**kwargs)
        ...
        for kwargs in invalid_kwargs:
            with self.assertRaises(AssertionError): EventStreamDatasetConfig(**kwargs)
    ...
    def test_to_and_from_dict(self):
        ...
        nontrivial_measurement_configs = {
            'col_A': MeasurementConfig(
                ...
                measurement_metadata=pd.DataFrame(
                    {'A': [1, 2, 3], 'B': ['a', 'b', 'c']}, index=pd.Index([2, 4, 6], name='index_var'),
                )
            ),
        ...
        cases = [{
            ...
            'config': EventStreamDatasetConfig(
                measurement_configs = nontrivial_measurement_configs
            ),
            'want_dict': {
                **default_dict,
                'measurement_configs': {
        ...
        for C in cases:
            with self.subTest(C['msg']):
                ...
                got_config = EventStreamDatasetConfig.from_dict(C['want_dict'])
                self.assertEqual(C['config'], got_config)

    def test_from_simple_args(self):
        want_config = EventStreamDatasetConfig(
            measurement_configs = {
        ...
        got_config = EventStreamDatasetConfig.from_simple_args(
