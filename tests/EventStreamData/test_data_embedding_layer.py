...
from EventStream.EventStreamData.data_embedding_layer import (
    DataEmbeddingLayer,
...
class TestDataEmbeddingLayer(MLTypeEqualityCheckableMixin, unittest.TestCase):
    ...
    `EventStreamPytorchDataset` object's `collate` function.
    """
    ...
    def test_constructs(self):
        ...
        cases = [
            ...
            },
            {
                ...
                'params': {**valid_params, 'categorical_embedding_dim': 2, 'numerical_embedding_dim': 2},
            },
            {
                'msg': (
            ...
            },
            {
                ...
                'params': {**valid_params, 'numerical_embedding_dim': 2, 'categorical_embedding_dim': 4.5},
                'should_raise': TypeError,
            },
            {
                'msg': "`DataEmbeddingLayer` should error if `categorical_embedding_dim` is negative.",
                'params': {**valid_params, 'categorical_embedding_dim': -1, 'numerical_embedding_dim': 2},
                'should_raise': ValueError,
            },
            {
                'msg': "`DataEmbeddingLayer` should error if `numerical_embedding_dim` is not an int.",
                'params': {**valid_params, 'numerical_embedding_dim': 4.5, 'categorical_embedding_dim': 2},
                'should_raise': TypeError,
            },
            {
                'msg': "`DataEmbeddingLayer` should error if `numerical_embedding_dim` is negative.",
                'params': {**valid_params, 'numerical_embedding_dim': -1, 'categorical_embedding_dim': 2},
                'should_raise': ValueError,
            },
            {
                'msg': (
            ...
            },
            {
                ...
                'params': {
                    **valid_params,
                    'split_by_measurement_indices': 'foo'
                },
                'should_raise': TypeError,
            },
        ]
        ...
        cases = [
            {
                ...
                'params': {**valid_params, 'static_embedding_mode': StaticEmbeddingMode.CONCAT_ALL},
                'batch': default_batch,
                'want': torch.Tensor([
                    [
                        [0, 1, -1, 1, 0, 1, 0, 0],
