...
class VisualizationConfig(JSONableMixin):
    split_subject_plots_by_static_covariates: List[str] = dataclasses.field(default_factory=list)
    split_subject_plots_by_dynamic_last_seen_covariates: List[
        Union[str, Tuple[str, Dict[str, Set[str]]]]
    ...
    def __post_init__(self):
        ...
        if self.plot_by_age and self.age_col is None:
            raise ValueError("Can't plot by age if age_col is unspecified!")

    @property
    def plot_spec(self):
        plots = []
        if self.plot_by_time:
            suffix = 'in dataset @ time'
            plots.extend([
                {'x': 'time', 'y': 'n_subjects', 'title': f"# of subjects {suffix}"},
                {'x': 'time', 'y': 'n_total_events', 'title': f"# of total events {suffix}"},
                {'x': 'time', 'y': 'n_total_measurements', 'title': f"# of total measurements {suffix}"},
                {
                    'x': 'time', 'y': 'n_events_per_subject', 'z': 'n_subjects',
                    'title': f"# of events per subject {suffix}"
                },
                {
                    'x': 'time', 'y': 'n_measurements_per_event', 'z': 'n_events',
                    'title': f"# of measurements per event {suffix}"
                },
                {
                    'x': 'time', 'y': 'n_measurements_per_subject', 'z': 'n_subjects',
                    'title': f"# of measurements per subject {suffix}"
                },
            ])
            if self.age_col is not None:
                plots.append(
                    {'x': 'time', 'y': 'subject_age', 'z': 'n_subjects', 'title': f"age of subjects {suffix}"}
                )

        if self.plot_by_age:
            suffix = 'subjects @ age'
            plots.extend([
                {'x': 'subject_age', 'y': 'n_subjects', 'title': f"# of {suffix}"},
                {'x': 'subject_age', 'y': 'n_total_events', 'title': f"# of total events for {suffix}"},
                {'x': 'time', 'y': 'n_total_measurements', 'title': f"# of total measurements {suffix}"},
                {
                    'x': 'time', 'y': 'n_events_per_subject', 'z': 'n_subjects',
                    'title': f"# of events per subject {suffix}"
        ...
        plots.append({'histogram': 'event_type', 'title': 'Event Types'})
