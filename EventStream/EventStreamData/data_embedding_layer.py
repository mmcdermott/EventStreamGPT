...
class DataEmbeddingLayer(torch.nn.Module):
    """
    This class efficiently embeds an EventStreamPytorchDataset batch's metadata into a fixed-size embedding
    ...
    def __init__(
        ...
        `EventStreamPytorchDataset` object is structured.
    ...
    def static_embedding(self, batch: EventStreamPytorchBatch) -> torch.Tensor:
    ...
    def split_batch_into_measurement_index_buckets(
        ...
        Args:
            `batch` (`EventStreamPytorchBatch`): A batch of data.
    ...
    def dynamic_embedding(self, batch: EventStreamPytorchBatch) -> torch.Tensor:
